...
@interface BNTabBarViewZero()<UIScrollViewDelegate>

@property (strong, nonatomic, readwrite) NSArray *items;

@end

@implementation BNTabBarViewZero

+ (BNTabBarViewZero *)viewRect:(CGRect)frame items:(NSArray *)items{
    BNTabBarViewZero * view = [[BNTabBarViewZero alloc]initWithFrame:frame items:items];
    return view;
...
-(void)updateTableWithPageNumber:(NSInteger)pageNumber{
    ...
    if (self.block) {
        self.block(self, self.tableView, pageNumber);
    }
}
