...
static CGFloat const kPopoverViewCellHeight = 45.f;   ///< cell指定高度
...
- (void)showToView:(UIView *)pointView items:(NSArray<NSDictionary *> *)items{
    
    NSMutableArray * actionList = [NSMutableArray arrayWithCapacity:0];
    for (NSDictionary * dict in items) {

        PopoverAction *action = nil;
        if (dict[kItem_image]) {
            action = [PopoverAction actionWithImage:[UIImage imageNamed:dict[kItem_image]] title:dict[kItem_title] handler:^(PopoverAction *action) {
                if (self.blockView) {
                    self.blockView(self, action, dict);
                }
             }];
        }
        else{
            action = [PopoverAction actionWithTitle:dict[kItem_title] handler:^(PopoverAction *action) {
                if (self.blockView) {
                    self.blockView(self, action, dict);
                }
    ...
    [self showToView:pointView withActions:actionList.copy];
}
