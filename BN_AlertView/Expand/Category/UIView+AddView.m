...
+ (void)renderImgView:(UIImageView *)imgView image:(id)image imageDefault:(NSString *)imageDefault{
    
    if (![image validObject]) {
        imgView.image = [UIImage imageNamed:imageDefault];
        return;
    }
    
    if ([image isKindOfClass:[NSString class]]) {
//        if ([image containsString:@"/"]) image = [WHKRequestHelper getImageUrlWithImageName:image];
        if ([image hasPrefix:@"http"]) {
            imgView.image = [UIImage imageNamed:imageDefault];//占位

            imageDefault = imageDefault ? : kIMAGE_default_failed_S;//
            [imgView sd_setImageWithURL:image placeholderImage:[UIImage imageNamed:imageDefault]];//
...
+ (void)renderImgView:(UIImageView *)imgView image:(id)image{
    [UIView renderImgView:imgView image:image imageDefault:kIMAGE_default_User];
...
+ (UIImageView *)createImageViewWithRect:(CGRect)rect image:(id)image tag:(NSInteger)tag patternType:(NSString *)patternType{
    ...
    if ([image isKindOfClass:[NSString class]]) {
        ...
        [UIView renderImgView:imageView image:image imageDefault:nil];
