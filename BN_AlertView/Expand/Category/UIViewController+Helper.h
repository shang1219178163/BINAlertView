 ...
 */
- (void)showAlertWithTitle:(nullable NSString *)title placeholderList:(NSArray *_Nullable)placeholderList msg:(NSString *_Nullable)msg actionTitleList:(NSArray *_Nonnull)actionTitleList handler:(void(^)(UIAlertController * _Nonnull alertVC, UIAlertAction * _Nullable action))handler;
...
/**
 法律条款/合作协议
 */
- (void)pushToRichTextDisplayType:(NSString *_Nonnull)type title:(NSString *)title;
