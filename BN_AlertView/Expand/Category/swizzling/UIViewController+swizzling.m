...
- (void)createBtnBack{
    ...
    UIButton * btnBack = [self createBarBtnItemWithTitle:nil imageName:nil isLeft:YES target:self aSelector:selector isHidden:NO];
    if ([self respondsToSelector:selector]) {
        
    }
    else{
        [btnBack removeTarget:self action:selector forControlEvents:UIControlEventTouchUpInside];
        [btnBack addTarget:self action:@selector(handleActionBtnBack:) forControlEvents:UIControlEventTouchUpInside];
        
    }
}

- (void)handleActionBtnBack:(UIButton *)sender{
